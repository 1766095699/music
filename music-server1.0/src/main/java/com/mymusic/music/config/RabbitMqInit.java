...
public class RabbitMqInit {
    ...
    public SimpleMessageListenerContainer payMessageListenerContainer(@Autowired @Qualifier(value = "connectionFactory") ConnectionFactory connectionFactory,
                                                                      SongPayMessageListener songPayMessageListener) {

        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer(connectionFactory);
//        container.setQueues(queue2());
        container.setQueueNames("queue.music.song");
        container.setExposeListenerChannel(true);
        container.setAcknowledgeMode(AcknowledgeMode.MANUAL);
        container.setMessageListener(songPayMessageListener);
        /** 设置消费者能处理消息的最大个数 */
    ...
    public Queue queue1(){
        ...
        return new Queue("queue.music.song",true,false,false,arguments);
    }
    @Bean
    public Binding binding1(){
        return new Binding("queue.music.song", Binding.DestinationType.QUEUE ,"exchange.music.pay" ,"key.order.pay" ,null);
    }
}
